...
async function collectEntries(dir: string, out: string, isRoot: boolean = true, result: IResourceEntries[]): Promise<void> {
  const files = await readDir(dir);
  const item: IResourceEntries = {
    name: isRoot ? "VectorResources" : toCamelCase(dir.split(path.sep).pop()!) + "Resources",
    entries: [],
  };
...
function getEntryDeclaration(entry: FileEntry): string {
  if (cmdOptions.ts) {
    return `  static readonly ${entry.variableName}: string = "${entry.relativeResourcePath}"`;
  } else {
    return `  static ${entry.variableName} = "${entry.relativeResourcePath}"`;
  }
}
